/**
 * Global exception handler for handling exceptions in the doctor service.
...
public class GlobalExceptionHandler {
	...
	/**
	 * Handles exceptions of type {@link ParseDateException}.
	 * This method returns a customized {@link ResponseEntity} with an error message and HTTP status code.
	 *
	 * @param ex The {@link ParseDateException} to handle.
	 * @return A {@link ResponseEntity} containing the error response.
	 */
	@ExceptionHandler(ParseDateException.class)
	public ResponseEntity<ApiResponse> parseDateExceptionHandler(ParseDateException ex){
		String message = ex.getMessage();
		return new ResponseEntity<ApiResponse>(
				new ApiResponse(false, message, null),
